...
function displayTemples(filter = "Home"){
    ...
    if (!container) {
        console.error("templeContainer div NOT found!");
        return;}
    //clear content in the templeContainer
    container.innerHTML = "";
    ...
    } else if (filter.toUpperCase() === "LARGE") {
        filtered = temples.filter(t => t.area > 90000);
    } else if (filter.toUpperCase() === "SMALL") {
    ...
    filtered.forEach(temple => {
        ...
        <img src="${temple.imageUrl}" alt="Image of ${temple.templeName} temple">
...
document.querySelectorAll(".navigation a").forEach(link => {
    link.addEventListener("click", (event) => {
        ...
        displayTemples(filter === "Home" ? "Home" : filter
